...
def export(data, pats):
    ...
    for ind, i in enumerate(temp_data):
        if (count<len(categorized_data)-1 and float(i[3])==float(categorized_data.T[0][count])):
            raw_data[ind+1][23] = categorized_data.T[1][count]
            count += 1
        else:
            ind += 1
    return raw_data
...
df.to_csv("Final_Classifications/CNN/classified_data_withoutclass5.csv", index=False, header = False)
#df.to_json("Final_Classifications/CNN/categorized_data.json")
